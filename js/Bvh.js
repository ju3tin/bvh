...
BVH.Reader.prototype = {
    ...
    parseData:function(data){
		...
		while (!done) {
			...
			case 'ROOT':
				...
				//var pos_helper = new THREE.Vector3( 0, -10, 0 );
				//this.root.position.copy(pos_helper);
	...
	{
		//this.play = false;
		//this.frameHelper = (f * this.numFrames).toFixed(0);
		//this.frame = (f * this.numFrames).toFixed(0);
		//this.play = function() { this.oldFrame = this.frame; this.startTime = Date.now(); this.play = true;};
		
		//414
		this.startTime = Date.now(); 
		this.frame = Math.floor(f * this.numFrames); 
		this.oldFrame = Math.floor(f * this.numFrames); 
		this.animate();
		//alert(this.oldFrame);
		//this.play = true;
		//this.play = true;
    ...
    animate:function(){
		...
		setProgBar(this.frame / this.numFrames)
		...
		for ( var i = 0, len = ref.length; i < len; i++) {
			...
			switch ( ch.prop ) {
				...
				case 'Zrotation':
					...
					break;
				case 'Xposition':
				    if(isRoot) ch.node.position.x = ch.node.offset.x + (parseFloat(this.data[n]) * this.scale) + this.position.x;
					else ch.node.position.x = ch.node.offset.x + parseFloat(this.data[n]);
					break;
				case 'Yposition':
				    if(isRoot) ch.node.position.y = ch.node.offset.y + (parseFloat(this.data[n]) * this.scale) + this.position.y;
					else ch.node.position.y = ch.node.offset.y + parseFloat(this.data[n]);
					break;
				case 'Zposition':
				    if(isRoot) ch.node.position.z = ch.node.offset.z + (parseFloat(this.data[n]) * this.scale) + this.position.z;
    ...
    update:function(){
    	if ( this.play ) { 
			...
			if(this.oldFrame!==0)
			{
				this.frame += this.oldFrame;
				//alert(this.frame);
			}
			/*if (this.frameHelper !==0)
			{
				alert(this.frame);
				this.frame = this.frameHelper;
				this.frameHelper = 0;
				alert(this.frame);
			}*/
